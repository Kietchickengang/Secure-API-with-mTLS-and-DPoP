...
function verifyMTLSBinding(certHeader, accessTokenPayload) {
  ...
  if (!tokenThumbprint) throw new Error('mTLS Binding Failed: no cnf.x5t#S256 in token');
  if (!certThumbprint) throw new Error('mTLS Binding Failed: no cert thumbprint from Envoy');

  const a = Buffer.from(certThumbprint);
  const b = Buffer.from(tokenThumbprint.toLowerCase());
  if (a.length !== b.length || !crypto.timingSafeEqual(a, b)) throw new Error('mTLS Certificate Binding Mismatch');
